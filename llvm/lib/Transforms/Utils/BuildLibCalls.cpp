...
Value *llvm::castToI8Ptr(Value *V, IRBuilderBase &B) {
...
Value *llvm::emitMemCpyChk(Value *Dst, Value *Src, Value *Len, Value *ObjSize,
  ...
  Dst = castToI8Ptr(Dst, B);
  Src = castToI8Ptr(Src, B);
...
Value *llvm::emitMemChr(Value *Ptr, Value *Val, Value *Len, IRBuilderBase &B,
  ...
  return emitLibCall(
      ...
      {castToI8Ptr(Ptr, B), Val, Len}, B, TLI);
}
...
Value *llvm::emitMemCmp(Value *Ptr1, Value *Ptr2, Value *Len, IRBuilderBase &B,
  ...
  return emitLibCall(
      ...
      {B.getInt8PtrTy(), B.getInt8PtrTy(), DL.getIntPtrType(Context)},
      {castToI8Ptr(Ptr1, B), castToI8Ptr(Ptr2, B), Len}, B, TLI);
}
...
Value *llvm::emitFPutC(Value *Char, Value *File, IRBuilderBase &B,
  ...
  if (Char->getType()->isByteTy())
    V = B.CreateByteCast(Char);
  V = B.CreateIntCast(V, B.getInt32Ty(), /*isSigned=*/true, "chari");
...
Value *llvm::emitFWrite(Value *Ptr, Value *Size, Value *File, IRBuilderBase &B,
  ...
  CallInst *CI =
      B.CreateCall(F, {castToI8Ptr(Ptr, B), Size,
