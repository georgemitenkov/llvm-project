...
Instruction *InstCombinerImpl::SimplifyAnyMemTransfer(AnyMemTransferInst *MI) {
  ...
  IntegerType *ByteType = ByteType::get(MI->getContext(), Size << 3);
  Type *NewSrcPtrTy = PointerType::get(ByteType, SrcAddrSp);
  Type *NewDstPtrTy = PointerType::get(ByteType, DstAddrSp);
...
Instruction *InstCombinerImpl::SimplifyAnyMemSet(AnyMemSetInst *MI) {
  ...
  if (Len <= 8 && isPowerOf2_32((uint32_t)Len)) {
    Type *BTy = ByteType::get(MI->getContext(), Len * 8); // n=1 -> i8.
    ...
    Constant *C = ConstantExpr::getBitCast(ConstantInt::get(ITy, Fill), BTy);
    StoreInst *S = Builder.CreateStore(C, Dest, MI->isVolatile());
