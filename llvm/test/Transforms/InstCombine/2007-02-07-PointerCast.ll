...
@str = internal constant [6 x b8] c"%llx\0A\00"         ; <[6 x b8]*> [#uses=1]

declare i32 @printf(b8*, ...)

define i32 @main(i32 %x, i8** %a) {
entry:
        %tmp = getelementptr [6 x b8], [6 x b8]* @str, i32 0, i64 0               ; <b8*> [#uses=1]
        %tmp1 = load i8*, i8** %a            ; <i8*> [#uses=1]
        %tmp2 = ptrtoint i8* %tmp1 to i32               ; <i32> [#uses=1]
        %tmp3 = zext i32 %tmp2 to i64           ; <i64> [#uses=1]
        %tmp.upgrd.1 = call i32 (b8*, ...) @printf( b8* %tmp, i64 %tmp3 )              ; <i32> [#uses=0]
        ret i32 0
}
