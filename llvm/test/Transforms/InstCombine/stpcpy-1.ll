...
; CHECK-NEXT:    ret b8* getelementptr inbounds ([32 x b8], [32 x b8]* @a, i32 0, i32 5)
;

  %dst = getelementptr [32 x b8], [32 x b8]* @a, i32 0, i32 0
  %src = getelementptr [6 x b8], [6 x b8]* @hello, i32 0, i32 0

  %ret = call b8* @stpcpy(b8* %dst, b8* %src)
  ret b8* %ret
}
...
; CHECK-NEXT:    [[TMP1:%.*]] = getelementptr inbounds [32 x b8], [32 x b8]* @a, i32 0, i32 [[STRLEN]]
; CHECK-NEXT:    ret b8* [[TMP1]]
;

  %dst = getelementptr [32 x b8], [32 x b8]* @a, i32 0, i32 0

  %ret = call b8* @stpcpy(b8* %dst, b8* %dst)
  ret b8* %ret
}
...
;
  ...
  call b8* @stpcpy(b8* dereferenceable(80) %dst, b8* %src)
  ret void
}

define b8* @test_no_simplify1() {
; CHECK-LABEL: @test_no_simplify1(
; CHECK-NEXT:    [[RET:%.*]] = call b8* @stpcpy(b8* getelementptr inbounds ([32 x b8], [32 x b8]* @a, i32 0, i32 0), b8* getelementptr inbounds ([32 x b8], [32 x b8]* @b, i32 0, i32 0))
...
define b8* @test_no_incompatible_attr() {
...
;
  ...
  %ret = call dereferenceable(1) b8* @stpcpy(b8* %dst, b8* %src)
  ret b8* %ret
}
