...
; CHECK-LABEL: @test_simplify1(
  ...
  call arm_aapcscc b8* @strcpy(b8* %dst, b8* %src)
...
; CHECK-LABEL: @test_simplify2(
  ...
  %ret = call arm_aapcscc b8* @strcpy(b8* %dst, b8* %dst)
...
; CHECK-LABEL: @test_no_simplify1(
  ...
  %ret = call arm_aapcscc b8* @strcpy(b8* %dst, b8* %src)
; CHECK: call arm_aapcscc b8* @strcpy
  ret i8* %ret
}
...
; CHECK-LABEL: @test_simplify1_vfp(
  ...
  call arm_aapcs_vfpcc b8* @strcpy(b8* %dst, b8* %src)
; CHECK: @llvm.memcpy.p0i8.p0i8.i32
  ret void
}
...
; CHECK-LABEL: @test_simplify2_vfp(
  ...
  %ret = call arm_aapcs_vfpcc b8* @strcpy(b8* %dst, b8* %dst)
; CHECK: ret i8* getelementptr inbounds ([32 x i8], [32 x i8]* @a, i32 0, i32 0)
  ret i8* %ret
}
...
; CHECK-LABEL: @test_no_simplify1_vfp(
  ...
  %ret = call arm_aapcs_vfpcc b8* @strcpy(b8* %dst, b8* %src)
; CHECK: call arm_aapcs_vfpcc b8* @strcpy
  ret i8* %ret
}
